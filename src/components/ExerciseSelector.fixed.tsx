...
export interface ExerciseSelectorProps {
  selectedExercises: { exerciseId: string; exercise: Exercise; sets: Set[] }[];
  onExercisesChange: (exercises: { exerciseId: string; exercise: Exercise; sets: Set[] }[]) => void;
  onClose: () => void;
}
...
export const ExerciseSelector: FC<ExerciseSelectorProps> = observer(({
  ...
  const filteredExercises = appStore.exercises.exercises.filter((exercise: any) =>
  ...
  return (
    <ModalRoot
      activeModal={showSetModal ? 'set-modal' : null}
          ...
          <Group header={<Header>Выбранные упражнения</Header>}>
            <Div>
              {selectedExercises.map((exerciseItem) => (
                <Card key={exerciseItem.exerciseId} mode="outline" style={{ padding: 12, marginBottom: 8 }}>
