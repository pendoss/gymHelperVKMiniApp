...
import { Icon28EditOutline, Icon28ClockOutline, Icon28VideoOutline, Icon28CheckCircleOutline } from "@vkontakte/icons";
...
export const ExerciseDetail: FC<ExerciseDetailProps> = observer(({ id }) => {
    ...
    const exercise = store.exercises.find((e: Exercise) => String(e.id) === String(exerciseId));

    if (!exercise) {
        return (
            <Panel id={id}>
                <PanelHeader before={<PanelHeaderBack onClick={() => routeNavigator.back()} />}>Упражнение</PanelHeader>
                <Div>
    ...
    const sortedWorkoutGroups = workoutGroups.sort(
    ...
    const calculateExerciseStats = () => {
        ...
        const repsArr = allSets.map((s) => s.reps).filter((v): v is number => typeof v === "number" && !isNaN(v));
        const repsRange = repsArr.length > 0 ? `${Math.min(...repsArr)}-${Math.max(...repsArr)}` : "8-12";
        const avgReps = repsArr.length ? Math.round(repsArr.reduce((a, b) => a + b, 0) / repsArr.length) : undefined;

        // Вес
        const weights = allSets.map((s) => s.weight).filter((v): v is number => typeof v === "number" && !isNaN(v));
        ...
        const hasRange =
            typeof exercise.minWeight === "number" && typeof exercise.maxWeight === "number" && exercise.maxWeight > 0;
        const weightRange = hasRange
            ? `${exercise.minWeight}-${exercise.maxWeight} кг`
            : weights.length
            ? `${Math.min(...weights)}-${Math.max(...weights)} кг`
            : "60-80 кг";
        ...
        // Длительность сета
        const durations = allSets.map((s) => s.duration).filter((v): v is number => typeof v === "number" && !isNaN(v));
        ...
        return {
            ...
            difficulty,
            muscleGroups:
                exercise.muscleGroup && exercise.muscleGroup.length > 0 ? exercise.muscleGroup : ["Не указано"],
        };
    };
    ...
    const handleSetSelectionComplete = (selectedSets: ExerciseSet[]) => {
        ...
        routeNavigator.push("/workout-create");
    };
        ...
        <Panel id={id}>
                ...
                <div style={{ padding: "0 16px" }}>
                    ...
                    >
                        <Card
                            ...
                            style={{ padding: 16, textAlign: "center", alignContent: "center" }}
                        >
            ...
            <Group header={<Header size="s">Пошаговая техника выполнения</Header>}>
                <Div>
                    {exercise.steps && exercise.steps.length > 0 ? (
                        ...
                        >
                            <Text>Пошаговая техника не добавлена</Text>
                        </Card>
                    )}
                </Div>
            </Group>
            <Group header={<Header size="s">💡 Советы и рекомендации</Header>}>
                <Div>
                    {exercise.recommendations && exercise.recommendations.length > 0 ? (
                    ...
                    ) : exercise.recommendations && Array.isArray(exercise.recommendations) ? (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                            {exercise.recommendations.map((recomendation: string, index: number) => (
                                <Card
                                    key={index}
                                    mode="outline"
                                    style={{
                        ...
                        >
                            <Text>Рекомендации не добавлены</Text>
                        </Card>
                    )}
                </Div>
            </Group>
