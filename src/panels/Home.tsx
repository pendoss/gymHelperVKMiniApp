    ...
    <Panel id={id}>
      ...
      >
        <span className="train-sync-gradient-text">TrainSync</span>
            ...
            {fetchedUser && (
        <Group header={<Header className="enhanced-header">Добро пожаловать!</Header>} className="group" onClick={handleProfileClick}>
          <Div>
            <Cell
      ...
      {upcomingWorkouts.length > 0 && (
        <Group header={<Header className="enhanced-header">Предстоящие тренировки</Header>} className="enhanced-group">
          <Div>
            {upcomingWorkouts.map((workout) => (
              <Cell
                ...
                className="enhanced-cell"
                multiline
                subtitle={`${new Date(workout.date).toLocaleDateString('ru')} • ${workout.time} • ${workout.gym} • ${workout.exercises.length} упражнений`}
                style={{ 
                  padding: '16px',
                  marginBottom: '8px',
                  cursor: 'pointer'
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                  <h3 style={{ fontSize: 16, fontWeight: 600 }}>{workout.title}</h3>
                  <p>{workout.time}</p>
                </div>
              </Cell>
            ))}
      ...
      <Group header={<Header className="enhanced-header">Календарь тренировок</Header>} className="enhanced-group">
        <Div>
